...
<body>
	<table id="dg" class="" title="列表" style="width: 700px; height: 300px">
		<thead>
			<tr>
				...
				<th data-options="field:'title',width:180">标题</th>
				<th data-options="field:'content',width:380">内容</th>
				<th data-options="field:'createTime',width:180">发布时间</th>
			</tr>
		</thead>
	</table>
	...
	<div id="toolbar">
		...
		<div>
			标题: <input class="easyui-textbox" id="paramTitle">
			内容: <input class="easyui-textbox" id="paramContent">
	...
	<div id="dlg" class="easyui-dialog" title="数据参数" data-options="modal:true" style="width: 350px; height: 200px;" closed="true" buttons="#dlg-buttons">
		<form method="post" id="fm">
			<table cellpadding="5">
	    		...
	    		<tr>
	    			<td>通知标题:</td>
	    			<td><input class="easyui-textbox" type="text" name="title" maxlength="39" required="true" missingMessage="标题必须填写"></input></td>
	    		</tr>
	    		<tr>
	    			<td>通知内容:</td>
	    			<td><input class="easyui-textbox" required="true"  maxlength="1000" type="text" name="content" missingMessage="内容必须填写"></input></td>
	    		</tr>
	    	</table>
		</form>
	</div>
	...
	function loadDataGrid(queryConditon){
		 $('#dg').datagrid({  
	            ...
	            url:'noticeController.do?datagrid',//url调用Action方法  
	            loadMsg : '数据装载中......',  
		...
		function edit(){
			...
			if (row.length == 1){
				...
				$('#fm').form('load',row[0]);
				$('#id').val(row[0].id);
				$('#title').textbox('setValue',row[0].title);
				$('#content').textbox('setValue',row[0].content);
				url = 'noticeController.do?update';
			}else{
				$.messager.show({
					title: '提示信息',
					msg: '请选择一行数据再进行编辑！'
		...
		function doSearch(){
			var queryCondition = {'title':$('#paramTitle').val(),'content': $('#paramContent').val()};
			loadDataGrid(queryCondition);
		}
		function doReset(){
			$('#paramTitle').textbox('setValue','');
			$('#paramContent').textbox('setValue','');
			loadDataGrid(null);
		}
		function del(){
			...
			if (rows.length > 0){
				$.messager.confirm('提示信息','确定删除?',function(r){
					if (r){
						...
						$.ajax({
							url : 'noticeController.do?delete',
							type : 'post',
							data : {
								ids : ids.join(',')
							},
